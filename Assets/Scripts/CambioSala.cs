...
{	
	...
	*/
    ...
    {		
		...
		if(col.tag == "Player"){			
			EliminarObjectosExistentes();			
			// Salas mínimas por las que tendra que pasar 4 y máximo por 8 salas
			if(salasJugadas > 4)
				continua = Continuacion();
			// Carga la sala final					
			if(!continua || salasJugadas >= 8){
	...
	*/
	void CrearObjetosSala(){
		if(objetoSala != null){		
			...
			Vector3 pos4 = new Vector3(random.Next(80, 740), random.Next(100, 400), 0f);
			Vector3 pos5 = new Vector3(random.Next(80, 740), random.Next(100, 400), 0f);
			Vector3 pos6 = new Vector3(random.Next(80, 740), random.Next(100, 400), 0f);
			...
			listaObjetos.Add((GameObject)Instantiate(objetoSala, pos4, Quaternion.identity));
			listaObjetos.Add((GameObject)Instantiate(objetoSala, pos5, Quaternion.identity));
			listaObjetos.Add((GameObject)Instantiate(objetoSala, pos6, Quaternion.identity));
		}
	}
	...
	*/
	void NuevaPosicion(GameObject sala){
		...
		switch(sala.name){
			case "Mapa Grass":nuevaPosicion = new Vector3(400f, 240f, 0f);break;
			case "Snow":nuevaPosicion = new Vector3(70f, 250f, 0f);break;			
			case "Desert":nuevaPosicion = new Vector3(410f, 150f, 0f);break;			
		}						
	}
}
